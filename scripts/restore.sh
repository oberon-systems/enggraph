...
if [ "$mode" = database ]; then
    ...
    # root_path is UNIQUE, so a second project already reading this path would
    # fail the insert halfway through. Say so before anything is deleted
    # rather than letting the transaction roll back with a constraint name.
    if [ -n "$collision" ]; then
