  ...
  return {
    tools: [
      ...
      },
      {
        name: "get_context",
        description:
          "Assemble the engineering context for a question in one call: the " +
          "hybrid search and the reranker pick the seeds, the graph is " +
          "expanded around them into what depends on them, the tests near " +
        ...
        inputSchema: {
          ...
          properties: {
            ...
            project_type: {
              type: "string",
              description:
                "Assemble from every project of this kind instead of one " +
                "project. Cannot be combined with a named project. Types " +
                "are " +
                PROJECT_TYPES,
            },
            query: {
              type: "string",
              description:
                "The question, in words. The identifiers in it are matched " +
                "literally as well",
            },
            token_budget: {
              type: "number",
              description:
                `How large the packet may be, estimated at four characters ` +
                `per token (default ${DEFAULT_TOKEN_BUDGET}, min ` +
                `${MIN_TOKEN_BUDGET}, max ${MAX_TOKEN_BUDGET}). The budget ` +
                `is spent on the search hits first, then on what depends on ` +
                `them, the tests, what they define, what they call and what ` +
                `they import, in that order`,
            },
            seeds: {
              type: "number",
              description:
                `How many search hits the expansion starts from (default ` +
                `${DEFAULT_SEEDS}, max ${MAX_SEEDS})`,
            },
            expand: {
              type: "object",
              description:
                "Hops to follow per relationship tier, 0 to switch one off " +
                `(max ${MAX_EXPAND_HOPS}). Defaults to one hop of each`,
              properties: {
                callers: { type: "number" },
                callees: { type: "number" },
                tests: { type: "number" },
            ...
            include_chunks: {
              type: "boolean",
              description:
                "Carry the source text of the matched chunks (default " +
                "true). false returns references, summaries and relations " +
                "only, which is far cheaper",
            },
            rerank: {
              type: "boolean",
              description:
...
function readExpand(args: Record<string, unknown> | undefined): ExpandOptions {
  ...
  const hops = (key: string, fallback: number): number =>
    readBounded(asked, key, fallback, 0, MAX_EXPAND_HOPS);
  return {
 ...
 */
/**
 * The scope shared by the two searches that may span the database.
 *
 * A named project is verified here, and naming a type alongside it is the one
 * combination both tools refuse: a type narrows a search across projects.
 */
async function readSearchScope(
  args: Record<string, unknown> | undefined,
  sessionProject: string | null,
): Promise<{ named: string | null; kind: string | null }> {
  ...
  return async (request: CallToolRequest): Promise<CallToolResult> => {
    ...
    try {
      ...
      if (name === "search_code") {
        ...
        const found = await hybridSearch(dbPool, { named, kind, query, limit });
        const ranked = rerank(found.rows, query, limit, useRerank);
      ...
      if (name === "get_context") {
        const query = requireString(args, "query");
        const { named, kind } = await readSearchScope(args, sessionProject);
        const packet = await buildContext(dbPool, {
          query,
          named,
          kind,
          seeds: readBounded(args, "seeds", DEFAULT_SEEDS, 1, MAX_SEEDS),
