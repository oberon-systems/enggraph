...
6. **Commitizen Standard:** All Git commits are made through `commitizen`, never by hand. This repository configures the `wyld-cz` adapter in `.cz.yaml`; where that adapter is not installed, commitizen's own conventional-commits rules apply instead and their message shape is the valid one.
...
- **Git Commit Format:** Generated by `commitizen` (`cz commit`), never written by hand. The adapter named in `.cz.yaml` decides the shape: `[<type>][<scope>]: <subject>` with `wyld-cz`, `<type>(<scope>): <subject>` with commitizen's own rules.
