"""Where a project reads its selection from, and what wins when several say.

The order is a file in the tree, then the project row, then the row of an
organization holding it, then the global default, then the built-in selection.
Both documents resolve independently, because a tree may ship one file and not
the other - and the origin is reported alongside the spec, since it is what
the dashboard shows and what an index run records.
"""
...
def origins(tmp_path: Path, cursor: FakeCursor) -> tuple[str, str]:
    """Resolve the selection of `mono` and return the two origins."""
    selection = resolve(cursor, "mono", str(tmp_path))
...
def test_a_tree_with_nothing_falls_back_to_the_built_in_selection(
    ...
    selection = resolve(FakeCursor(), "mono", str(tmp_path))
...
def test_an_organization_still_loses_to_a_file_in_the_tree(tmp_path: Path) -> None:
    """A repository shipping a pair keeps deciding its own index either way."""
    (tmp_path / KEEP_FILE).write_text("*.tf\n")
    cursor = FakeCursor(
        settings={"acme": ("*.hcl\n", None)},
        members=[("acme", "mono")],
    )
    assert origins(tmp_path, cursor)[0] == "file"
...
def test_a_document_of_only_comments_says_nothing(tmp_path: Path) -> None:
    ...
    assert origins(tmp_path, cursor) == ("organization", "default")


def test_the_pair_a_walk_reads_is_the_one_that_was_resolved(
    tmp_path: Path,
) -> None:
    """`specs` is what `enggraph.discovery` filters by, keep first."""
