"""Propose the selection for a tree, and verify the proposal.

Run as `python -m enggraph.bootstrap` against the same read-only mount the
indexer walks, by `make install`, and through `POST /projects/{p}/scan` when
the dashboard offers to regenerate one. Nothing here reaches the database, so
it runs before the stack is needed, and everything it claims about supported
file types is read from the parser tables rather than restated here.

The proposal is printed rather than stored: what accepts it decides where it
goes. Onboarding stores it on the project row, the dashboard hands it to an
editor, and a `.enggraph-keep` already in the tree is left exactly as it was - the
mount is read-only by contract, and a repository that ships one goes on
deciding its own index until that file is deleted.
"""
...
from enggraph.discovery import load_spec, present, to_spec, walk_selected
from enggraph.identifiers import project_name
from enggraph.parsers.compose import is_compose_name
from enggraph.parsers.registry import (
    EXTENSION_PARSERS,
    FILENAME_PARSERS,
    parser_class,
)
...
def keep_document(inventory: Inventory) -> list[str]:
    """Render the proposed .enggraph-keep."""
    lines = [
        "# What of this tree becomes a node in the code graph.",
        "#",
        "# Generated from the file types actually present here. It REPLACES",
        "# the default selection rather than adding to it, so deleting a line",
        "# removes that file type from the graph.",
    ]
...
def ignore_document(inventory: Inventory) -> list[str]:
    """Render the proposed .enggraph-ignore."""
    lines = [
        "# What is pruned from the walk, applied after the keep list.",
        "#",
        "# Generated. Additive on top of the built-in skip list (.git, .venv,",
        "# node_modules, dist, target and friends), so it only names what",
    ...
    lines.extend(
        [
            ...
            "# here is a rendered copy of one that lives in enggraph,",
    ...
    if inventory.generated:
        lines.extend(
            [
                ...
                "# The default selection skips these; an explicit keep",
    ...
    if candidates:
        ...
        # not make the whole .enggraph-ignore differ from what a re-run writes.
...
def verify(
    ...
    # mistake. A supported file .enggraph-keep never names is a gap in the
    # selection; one .enggraph-ignore drops was named on purpose - key material,
    ...
    lines.extend(
        detail("supported, and .enggraph-keep does not name it - a gap", missed)
    )
    lines.extend(
        detail("supported, and .enggraph-ignore drops it - on purpose?", pruned)
    )
    ...
    lines.extend(
        detail(
            "bulk directory, named in .enggraph-ignore as a comment to decide on",
...
def main() -> None:
    ...
    keep_file = present(root_path, KEEP_FILES)
    ignore_file = present(root_path, IGNORE_FILES)
    existing_keep = load_spec(root_path, keep_file) if keep_file else None
    existing_ignore = load_spec(root_path, ignore_file) if ignore_file else None
    sources = []
    if keep_file:
